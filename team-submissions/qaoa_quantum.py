...
def energy_batch_cupy(bitstrings_2d):
    """
    bitstrings_2d: array-like shape (B, N) con 0/1
    Devuelve energies shape (B,) usando CuPy (GPU).
    """
    import cupy as cp

    X = cp.asarray(bitstrings_2d, dtype=cp.int8)   # (B,N)
    B, N = X.shape
    sigma = 2 * X - 1                              # {-1,+1}

    E = cp.zeros((B,), dtype=cp.int32)
    for k in range(1, N):
        Ck = cp.sum(sigma[:, :N-k] * sigma[:, k:], axis=1)  # (B,)
        E += Ck * Ck

    return cp.asnumpy(E)  # regresamos a numpy para escoger argmin fácil


def best_from_sample_with_cupy(sample_result):
    """
    sample_result: resultado de cudaq.sample (histograma/dict-like)
    Devuelve: (best_bits_list, best_energy_int)
    """
    bitstrs = list(sample_result.keys())  # strings tipo "0101..."
    if len(bitstrs) == 0:
        raise RuntimeError("Sample result vacío.")

    bits_mat = np.array([[int(c) for c in s] for s in bitstrs], dtype=np.int8)  # (B,N)
    Es = energy_batch_cupy(bits_mat)  # energies para todos los bitstrings

    idx = int(np.argmin(Es))
    return bits_mat[idx].tolist(), int(Es[idx])
...
def qaoa_best_bitstring(
    ...
    *,
    ...
    best_bits, best_E = None, None
    if cupy:
        best_bits, best_E = best_from_sample_with_cupy(res)
    else:
        best_bits, best_E = None, None
